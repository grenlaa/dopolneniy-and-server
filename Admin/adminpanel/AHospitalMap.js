...
const HospitalMap = () => {
    ...
    async function click() {
       { change ?
        editMap("http://localhost:8081/api/EditHospitalMap/",id,title,descr,number,site,url,cord[0],cord[1]):
        createMap("http://localhost:8081/api/CreateHospitalMap",title,descr,number,site,url,cord[0],cord[1]);}
    ...
    async function Del(asd) {
        r_delete("http://localhost:8081/api/DelHospitalMap/"+id)
        setCreate(false)
        setChange(false)
    ...
    async function fetchPosts() {
        const res = await axios.get('http://localhost:8081/api/HospitalMap');
        setPoin(res.data);
        console.log(poin);
    }
